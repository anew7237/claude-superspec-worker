...
- **Mode B UPSTREAM_URL 為 location-aware default**(per Q1 Clarification + Edge Cases + 2026-05-04 cross-spec review 修正):依 wrangler dev 運行位置決定 — host 運行(WSL2 / Mac / Windows native)→ `http://localhost:8000`;dev container 內運行 → `http://host.docker.internal:8000`(Mac/Win Docker Desktop 原生支援;Linux Docker Desktop 4.0+ 亦支援;若 host 自架 Docker Engine 無 Docker Desktop 可能需 `--add-host=host.docker.internal:host-gateway` runArg)。`.dev.vars.example` 已採此規則。T036 SC-011 acceptance 即在 WSL2 host wrangler → `localhost:8000` 情境完成驗收
...
7. **Mode B UPSTREAM_URL 為 location-aware default**(Q1 Clarification + plan-level decision + 2026-05-04 cross-spec review 修正):依 wrangler dev 運行位置決定 — host(WSL2/Mac/Windows native)→ `http://localhost:8000`(T036 acceptance 即此情境);dev container 內 → `http://host.docker.internal:8000`(README 註明 Linux 自架 Docker Engine 無 Docker Desktop 需 runArg `--add-host=host.docker.internal:host-gateway`,Mac/Windows Docker Desktop 4.0+ 預設支援)。`.dev.vars.example` 已採此規則
