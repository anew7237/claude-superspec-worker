...
| `git` + `gh` | features:`git:1` + `github-cli:1` | SSH agent forwarding 顯式宣告於 `mounts`(`${localEnv:SSH_AUTH_SOCK}` → `/ssh-agent`),FR-014。 |
...
| SSH agent 未轉發(host 未跑 ssh-agent / mount 失敗) | `post-create.sh:222-232` SSH sanity check 印 WARN,告知 git push 將 fall back 至 HTTPS;不阻擋容器啟動 |
